...
func (node *BPlusTreeNode) insertInNode(key index.KeyType, value index.ValueType, index uint16) bool {
	// if key value all exist in node, then not insert
	if bytes.Equal(node.Keys[index], key) &&
		(bytes.Equal(node.Children[index], value) || bytes.Equal(node.Children[index+1], value)) {
		return false
	}
	// insert key
